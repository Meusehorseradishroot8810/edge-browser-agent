...
MAX_STEPS = 5; CTX_CHARS = 48000     # ~12-15k tokens of raw HTML
LOG = "logs/control_rawhtml.jsonl"; SESSION = None
...
for step in range(1, MAX_STEPS + 1):
    ...
    try:
        ...
        if not on_book:
            task = ("Find the book " + json.dumps(BOOK) + " on this site. If you see its link, return its href. "
                    "Otherwise return the href of a category or page link that could lead to it.")
            ...
            if "toscrape.com" not in absolute: raise ValueError("Off-site href: " + absolute)
        ...
        prompt = ("TASK: from the RAW HTML of this product page, extract price, star rating as a word (One..Five) and stock status. "
                  'Return only {"action":"report","price":"...","rating":"...","stock":"..."}. HTML=' + body)
        raw = llm(prompt, 120); print("Model:", raw[:300]); d = parse_choice(raw)
        checks = {k: (str(d.get(k, "")).strip().startswith(v) if k == "stock" else str(d.get(k, "")).strip() == v) for k, v in EXPECTED.items()}
